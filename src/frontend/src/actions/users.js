export const SET_USER = 'SET_USER'
export const SET_USER_POINTS = 'SET_USER_POINTS'
export const SET_ROLE = 'SET_ROLE'
export const SET_SOCIAL_AUTH = 'SET_SOCIAL_AUTH'
export const MENU_TOGGLE = 'MENU_TOGGLE'
export const ERROR_POPUP_OPEN = 'ERROR_POPUP_OPEN'
export const SET_USER_NAME = 'SET_USER_NAME'
export const SET_ERROR_MESSAGE = 'SET_ERROR_MESSAGE'
export const USER_LOGOUT  = 'USER_LOGOUT'
export const INITIAL_LOAD = 'INITIAL_LOAD'
export const SET_USER_PHOTO = 'SET_USER_PHOTO'
export const SET_CURRENT_CAR_PHOTO = 'SET_CURRENT_CAR_PHOTO'
export const SET_USER_MODERATION_ARRAY = 'SET_USER_MODERATION_ARRAY'
export const SET_PREVIOUS_ROUTE = 'SET_PREVIOUS_ROUTE'
export const SET_TOKEN_TIMEOUT = 'SET_TOKEN_TIMEOUT'
export const SET_TOKENS_FETCHING = 'SET_TOKENS_FETCHING'
export const REFRESH_TOKENS = 'REFRESH_TOKENS'


